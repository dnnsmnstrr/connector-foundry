...
| ![T-slot tab (hammer-head)](docs/img/extrusion2020_tab.png)<br>T-slot tab (hammer-head) | 2020 Extrusion | parametric | Unmeasured — verify against your hardware before trusting the fit. tab_len <= 6mm (the default) drops into the slot face-first and twists 90 degrees to lock, anywhere along the rail; longer heads can't tilt through the 6.2mm mouth and must be fed in endwise from an open end of the extrusion. |
| ![Snap-in clip](docs/img/extrusion2020_clip.png)<br>Snap-in clip | 2020 Extrusion | parametric | Unmeasured — verify against your hardware before trusting the fit. Push straight into the slot mid-rail, no end access needed; the legs flex through EX_SLOT_OPEN and the barbs spring out to grip EX_CHANNEL_W. Print in a flexible-enough filament (PETG/nylon) — stiff PLA legs may snap. |
