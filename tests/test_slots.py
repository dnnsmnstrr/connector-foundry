...
def test_declared_pitch_matches_the_geometry(part, render_dir):
    out_dir = render_dir / "slots" / part["id"].replace("/", "_")
    pitch = part["slots"]["pitch"]
    nx, ny = _grid_counts(part, out_dir)
    ...
    origin = _corner(part, origin_name, out_dir, "origin")
    x_neighbor = _corner(part, x_neighbor_name, out_dir, "x_neighbor")
    y_neighbor = _corner(part, y_neighbor_name, out_dir, "y_neighbor")
